...
public class StripeService {
    ...
    public String createPortalSession(final AccountEntity account) throws StripeException {
        log.info("Creating billing portal session for account: {}, customer: {}", account.getId(), account.getStripeCustomerId());
    ...
    public void updateExtraTunnels(final AccountEntity account, final int newCount) throws StripeException {
        ...
        if (subscriptionItemId != null) {
            if (newCount == 0) {
                ...
                log.info("Updating extra tunnels quantity to {} for subscription: {}", newCount, account.getStripeSubscriptionId());
                // Update quantity
                paramsBuilder.addItem(SubscriptionUpdateParams.Item.builder()
                        .setId(subscriptionItemId)
            ...
            log.info("Adding extra tunnels item (quantity: {}) to subscription: {}", newCount, account.getStripeSubscriptionId());
    ...
    private String getOrCreateCustomer(final AccountEntity account) throws StripeException {
        if (account.getStripeCustomerId() != null) {
            log.debug("Using existing Stripe customer {} for account {}", account.getStripeCustomerId(), account.getId());
